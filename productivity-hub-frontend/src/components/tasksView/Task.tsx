...
function Task({ task }: { task: Task }) {
  ...
  const handleDelete = () => {
    deleteTaskMutation({ variables: { id } }).then(() => {
    ...
    <Accordion>
      <AccordionSummary expandIcon={<ExpandMoreIcon />}>
        <Stack direction="row" justifyContent="space-between" sx={{minWidth: '100%'}}>
          ...
          <Typography variant="h6" sx={{opacity: 0.5, mr: 2}}>{dueDate}</Typography>
        </Stack>
      </AccordionSummary>
      <AccordionDetails>
        ...
        <Stack direction="row" justifyContent="end">
          <TaskFormDialog open={dialogOpen} setOpen={setDialogOpen} task={task} />
